				 ...
				 <div id="cart-header-<?php echo $key->cartid; ?>" class="cart-header">
					 ...
					 <div class="cart-sec simpleCart_shelfItem">
							<div class="cart-item cyc">
								 <img src="data:image/jpeg;base64,<?php echo base64_encode($key->img1); ?>" class="img-responsive" alt="">
							</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('button').click(function(){
			...
			$.ajax({
	          url: '<?php echo base_url('Ajax/updatecart/') ?>/' + cartid  +'/' + qty,
	          type: 'GET',
	          async: true,
	          beforeSend: function() {
	          	$('#messagebox').addClass('alert-info');
		        $('#messagebox').html('<h3>Please Wait...</h3>');
		      },
	          success: function(content) {
	            $('#messagebox').html('Update Cart Success!');
	            location.reload();
	          },
		...
		$('.close').click(function(){
			...
			$.ajax({
	          ...
	          async: true,
	          beforeSend: function() {
	          	$('#messagebox').show().addClass('alert-info').html('<h3>Please Wait...</h3>');
		      },
	          success: function(content) {
	            $('#messagebox').html('Item Removed!').delay(1500).fadeOut();
	            $('#cart-header-' + cartid).fadeOut('slow', function(){
					$('#cart-header-' + cartid).remove();
				});
	          },
	          error: function(request, status, error) {
	            $('#messagebox').html(error);
